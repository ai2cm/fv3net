...
# oro
_oro_data_axes_map = defaultdict(lambda: ('tile',
                                          'grid_yt',
                                          'grid_xt')
                                )
# fv_core
_fv_core_res_axes_map = defaultdict(lambda: ('tile',
                                             'forecast_time',
                                             'initialization_time',
                                             'Time',
                                             'pfull',
...
_fv_core_res_axes_map['phis'] = ('tile',
                                 'forecast_time',
                                 'initialization_time',
                                 'Time',
                                 'grid_yt',
                                 'grid_xt')
_fv_core_res_axes_map['u'] = ('tile',
                              'forecast_time',
                              'initialization_time',
                              'Time',
                              'pfull',
...
_fv_core_res_axes_map['v'] = ('tile',
                              ...
                              'grid_x')
# fv_srf_wnd
_fv_srf_wnd_res_axes_map = defaultdict(lambda: ('tile',
                                                'forecast_time',
                                                'initialization_time',
                                                'Time',
                                                'grid_yt',
...
# fv_tracer 
_fv_tracer_res_axes_map = defaultdict(lambda: ('forecast_time',
                                               'tile',
                                               'initialization_time',
                                               'Time',
                                               'pfull',
...
# sfc data
_sfc_data_axes_map = defaultdict(lambda: ('tile',
                                          'forecast_time',
                                          'initialization_time',
                                          'Time',
                                          'grid_yt',
...
for soil_var in ['stc', 'smc', 'slc']:
    _sfc_data_axes_map[soil_var] = ('tile',
                                    'forecast_time',
                                    'initialization_time',
                                    'Time',
                                    'soil_levels',
...
# phy data
_phy_data_axes_map = defaultdict(lambda: ('tile',
                                          'forecast_time',
                                          'initialization_time',
                                          'Time',
                                          'grid_yt',
...
_phy_data_axes_map['phy_f3d_01'] = ('tile',
                                    'forecast_time',
                                    'initialization_time',
                                    'Time',
                                    'pfull',
...
def open_fv_tracer(paths: list) -> xr.Dataset:
    ...
    '''
    input_ds = xr.open_mfdataset(paths=[paths[0][0]], concat_dim = ['initialization_time', 'tile'], combine='nested').drop(labels='sgs_tke')
    restart_ds = xr.open_mfdataset(paths=[paths[0][1]], concat_dim = ['initialization_time', 'tile'], combine='nested')
    return xr.concat([input_ds, restart_ds], dim='forecast_time')#.transpose((1, 0, 2))
...
def use_diagnostic_coordinates(ds : xr.Dataset, category : str, output_mapping : dict) -> xr.Dataset:
    '''
