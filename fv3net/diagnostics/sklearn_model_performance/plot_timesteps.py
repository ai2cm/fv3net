...
def _plot_daily_hourly_count(time_list, suptitle):
    ...
    ser.groupby([ser.dt.year, ser.dt.month, ser.dt.day]).count().plot(
        ax=axes[0], kind="bar", title=f"Daily count (total={len(ser)})"
    )
    ser.groupby(ser.dt.hour).count().plot(ax=axes[1], kind="bar", title="Hourly count")
