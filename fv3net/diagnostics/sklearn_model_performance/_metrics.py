...
def create_metrics_dataset(ds):
    ...
    ds_metrics["r2_dQ1_pressure_levels_sea"] = _r2_pressure_level_metrics(
        vcm.mask_to_surface_type(ds, "sea"), "dQ1"
    )
    ds_metrics["r2_dQ2_pressure_levels_sea"] = _r2_pressure_level_metrics(
        vcm.mask_to_surface_type(ds, "sea"), "dQ2"
    )
    ds_metrics["r2_dQ1_pressure_levels_land"] = _r2_pressure_level_metrics(
        vcm.mask_to_surface_type(ds, "land"), "dQ1"
    )
...
def _r2_global_values(ds):
    ...
    for var in ["net_heating", "net_precipitation"]:
        ...
        r2_summary[f"R2_sea_{var}_vs_target"] = r2_score(
            vcm.mask_to_surface_type(ds.sel(dataset=DATASET_NAME_FV3_TARGET), "sea")[
                var
            ].stack(sample=STACK_DIMS),
            vcm.mask_to_surface_type(ds.sel(dataset=DATASET_NAME_PREDICTION), "sea")[
                var
            ].stack(sample=STACK_DIMS),
            "sample",
        ).values.item()
        ...
        r2_summary[f"R2_land_{var}_vs_target"] = r2_score(
            vcm.mask_to_surface_type(ds.sel(dataset=DATASET_NAME_FV3_TARGET), "land")[
                var
            ].stack(sample=STACK_DIMS),
            vcm.mask_to_surface_type(ds.sel(dataset=DATASET_NAME_PREDICTION), "land")[
                var
            ].stack(sample=STACK_DIMS),
            "sample",
        ).values.item()
