...
PARENT_BUCKET = 'gs://vcm-ml-data/2019-10-28-X-SHiELD-2019-10-05-multiresolution-extracted/'
...
LOCAL_DIAG_TABLE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'diag_table')
...
def submit_jobs(timestep_list):
    ...
    for timestep in timestep_list[:1]:
        ...
        config['namelist']['fv_core_nml'].update({'external_eta': True, 'npz': 79, 'k_split': 1, 'n_split': 1})
        ...
        fv3config.run_kubernetes(
            config_location, outdir, DOCKER_IMAGE,# runfile=runfile_location,
            jobname=job_name, namespace="default",
            memory_gb=3.6, cpu_count=1, gcp_secret="gcp-key",
