...
def compute_recoarsened_budget_v2(
    ...
    middle = (begin + end) / 2
    storage = (end - begin) / dt

    delp = middle.delp
    delp_c = grid.weighted_block_average(middle.delp, area, factor=factor)
    omega = physics.omega

    # Collect all variables
    variables_to_average = {}
    for key in VARIABLES:
        variables_to_average[key] = physics[key]

    variables_to_average["storage_T"] = storage["T"]
    variables_to_average["storage_q"] = storage["sphum"]
