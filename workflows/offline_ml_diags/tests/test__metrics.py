...
from offline_ml_diags._metrics import (
    ...
    _calc_batch_metrics,
    calc_metrics,
    DATA_SOURCE_DIM, TARGET_COORD, PREDICT_COORD
)
...
def ds_mock():
    ...
    return xr.Dataset({"dQ1": da, "dQ2": da, "pressure_thickness_of_atmospheric_layer": delp})
...
def ds_calc_test(request):
    ...
    da_area = xr.DataArray(
        area, dims=["y", "x"], coords={"x": [0, 1], "y": [0]}
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            [[[[10.0, 20.0], [-10.0, -20.0]]], [[[11.0, 24.0], [-13.0, -24.0]]], [[1.0, 4.0]]],
            (1.0 + 4.0 - 3.0 - 4.0) / 4, 
            (1.0 * 0.4 + 4.0 * 0.4 - 3.0 * 1.6 - 4.0 * 1.6) / 4
        ),
        (
            [[[[10.0, 20.0], [-10.0, -20.0]]], [[[10.0, 20.0], [-10.0, -20.0]]], [[1.0, 4.0]]],
            0., 
            0.
        ),
       (
            [[[[10.0, 20.0], [-10.0, -20.0]]], [[[11.0, 24.0], [-13.0, -24.0]]], [[4.0, 4.0]]],
            (1.0 + 4.0 - 3.0 - 4.0) / 4, 
            (1.0 + 4.0 - 3.0 - 4.0) / 4,
        ),
        (
            [[[[np.nan, 20.0], [-10.0, -20.0]]], [[[11.0, 24.0], [-13.0, -24.0]]], [[4.0, 4.0]]],
            ( 4.0 - 3.0 - 4.0) / 3, 
            ( 4.0 - 3.0 - 4.0) / 3,
        ),
    ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            [[[[10.0, 20.0], [-10.0, -20.0]]], [[[11.0, 24.0], [-13.0, -24.0]]], [[1.0, 4.0]]],
            np.sqrt((1. + 16. + 9. + 16.)/4.),
            np.sqrt((1.* 0.4 + 16.* 0.4 + 9.*1.6 + 16.*1.6)/4.)
        ),
        (
            [[[[np.nan, 20.0], [-10.0, -20.0]]], [[[11.0, 24.0], [-13.0, -24.0]]], [[4.0, 4.0]]],
            np.sqrt(( 16. + 9. + 16.)/3.),
