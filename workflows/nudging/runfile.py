...
REFERENCE_DIR = (
    "gs://vcm-ml-data/"
    "2020-01-16-X-SHiELD-2019-12-02-pressure-coarsened-rundirs/restarts/C48/"
)
...
def nudge_to_reference(state, partitioner, timescales, timestep):
    reference = get_reference_state(
        state["time"], partitioner, only_names=timescales.keys()
    )
...
if __name__ == "__main__":
    ...
    for i in range(fv3gfs.get_step_count()):
        ...
        tendencies = nudge_to_reference(
            state, partitioner, nudging_timescales, timestep
        )
