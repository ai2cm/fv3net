...
@pytest.fixture()
def test_config():
    return {'experiment': {'steps_config': {'argo_step': {'argo': 'argo submit some.yaml',
    'args': {'--optional_arg': 'optional_arg_value',
     'required_arg': 'required_arg_value',
     'required_location_arg': {'location': 'data_location'}},
    'output_location': 'output_location'},
   'python_step': {'args': {'--optional_arg': 'optional_arg_value',
     'required_arg': 'required_arg_value',
     'required_location_arg': {'location': 'data_location'}},
...
def test__get_all_step_arguments(test_config):
    ...
    assert set(all_commands.keys())==set(test_config["experiment"]["steps_config"].keys())
    assert all_commands["argo_step"] == \
        "argo submit some.yaml -p required_arg=required_arg_value -p required_location_arg=data_location -p output-location=output_location --optional_arg optional_arg_value"
...
def test__resolve_command(test_config):
    assert _resolve_command(test_config["experiment"]["steps_config"]["argo_step"])=="argo"
    assert _resolve_command(test_config["experiment"]["steps_config"]["python_step"])=="command"
