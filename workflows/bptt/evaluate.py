...
if __name__ == '__main__':
    ...
    val_inputs, val_given_tendency, val_target_out = prepare_keras_arrays(
        arrays, model.input_scaler, model.tendency_scaler, model.prognostic_scaler, timestep_seconds
    )
    ...
    for i in range(1, val_base_out.shape[1]):
        ...
        baseline_out[:, i, :] = baseline_out[:, i-1, :] + timestep_seconds * val_given_tendency[:, i, :]
        new_state = model.model.predict([val_inputs[:, i, :], val_out[:, i, :]])
        tendency_out[:, i, :] = model.tendency_scaler.denormalize(new_state - val_out[:, i, :]) / timestep_seconds
        val_out[:, i, :] = new_state
    ...
    #     plot_single(tendency_out[i, 1:, :nz] * seconds_in_day, arrays.nudging_tendency[i, 1:, :nz] * seconds_in_day, "air_temperature (K/day)", ax[:2])
    #     plot_single(tendency_out[i, 1:, nz:] * seconds_in_day, arrays.nudging_tendency[i, 1:, nz:] * seconds_in_day, "specific_humidity (kg/kg/day)", ax[2:])
    #     plt.tight_layout()
    #     plt.show()

    print("all times:")
    r2_air_temperature = 1 - np.std(tendency_out[:, 1:, :nz] - arrays.nudging_tendency[:, 1:, :nz]) / np.std(arrays.nudging_tendency[:, 1:, :nz])
    r2_specific_humidity = 1 - np.std(tendency_out[:, 1:, nz:] - arrays.nudging_tendency[:, 1:, nz:]) / np.std(arrays.nudging_tendency[:, 1:, nz:])
    ...
    r2_air_temperature = 1 - np.std(norm_tendency_out[:, 1:, :nz] - norm_nudging_tendency[:, 1:, :nz]) / np.std(norm_nudging_tendency[:, 1:, :nz])
    r2_specific_humidity = 1 - np.std(norm_tendency_out[:, 1:, nz:] - norm_nudging_tendency[:, 1:, nz:]) / np.std(norm_nudging_tendency[:, 1:, nz:])
    print(f"r2 air_temperature: {r2_air_temperature}")
    print(f"r2 specific_humidity: {r2_specific_humidity}")
    ...
    r2_air_temperature = 1 - np.std(tendency_out[:, 1, nz-20:nz] - arrays.nudging_tendency[:, 1, nz-20:nz]) / np.std(arrays.nudging_tendency[:, 1, nz-20:nz])
    r2_specific_humidity = 1 - np.std(tendency_out[:, 1, -20:] - arrays.nudging_tendency[:, 1, -20:]) / np.std(arrays.nudging_tendency[:, 1, -20:])
    print(f"r2 air_temperature: {r2_air_temperature}")
    print(f"r2 specific_humidity: {r2_specific_humidity}")
