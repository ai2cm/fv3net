...
parser.add_argument('--grid-spec', default="gs://vcm-ml-data/2020-01-06-C384-grid-spec-with-area-dx-dy/grid_spec")
...
area_averages = (resampled  * resampled.area).sum(horz_dims) / resampled.area.sum(horz_dims)
for variable in area_averages:
    lower = variable.lower()
    diags[f'{lower}_global_avg'] = area_averages[variable].assign_attrs(ds[variable].attrs)
