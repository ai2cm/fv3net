...
if [[ $# != 1 ]]
then
    echo "usage: tests/nudge_to_obs_integration/run_test.sh <version>"
    exit 1
fi
...
name=n2o-integration-test-$random
...
cd tests/nudge_to_obs_integration

envsubst < "config_template.yaml" > "config.yaml"

argo submit --from workflowtemplate/nudge-to-obs -f config.yaml --name $name
