...
def test_not_finished_extract(timestep, result):
    ...
    output_prefix = "test_data_extract_check"
    fake_tstep_gcs_tar_url = f"gs://vcm-ml-code-testing-data/{timestep}.tar"
    is_not_finished = not_finished_with_tar_extract(
        fake_tstep_gcs_tar_url, output_prefix, num_tiles=1, num_subtiles=1
    )
