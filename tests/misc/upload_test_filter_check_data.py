...
from vcm.cloud.gcs import init_blob

timesteps = ["20160801.003000", "20160801.004500", "20160801.010000"]
num_tiles = 1
num_subtiles = 1
...
dst_template = (
    prefix_dst + "{res}/{timestep}/" "{timestep}.{domain}.tile{tile:d}.nc.{subtile:04d}"
)
...
for tstep, tile, subtile in items:
    ...
    domains = ["fv_core_coarse.res", "fv_srf_wnd_coarse.res", "fv_tracer_coarse.res"]
...
rm_sfc_file = dst_template.format(
    res="C3702", timestep=timesteps[1], domain="sfc_data", tile=1, subtile=0
)
rm_atm_file = dst_template.format(
    res="C384", timestep=timesteps[2], domain="fv_srf_wnd_coarse.res", tile=1, subtile=0
)
