...
# memoize for speed
get_restart_times = memoize(get_restart_times)
...
# ds = open_restarts_recursive(url)


# def _split_tile(url):
#     return url[: -len("tile1.nc")], int(url[-4])


# tuples = [_split_tile(url.item()) for url in ds.url]
# d.MultiIndex.from_tuples(tuples, names=["url", "tile"])
# new_idx = pd.MultiIndex.from_tuples(tuples, names=["url_step", "tile"])
# ds["url"] = new_idx

# unstacked = ds.unstack("url")
