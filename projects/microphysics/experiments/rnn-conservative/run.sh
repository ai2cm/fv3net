...
submit () {
    ...
    argo submit ../../train/argo.yaml \
        ...
        -p wandb-run-group="conservative-rnn" \
        -p tag="${tag}" \
        -p training-config="$(base64 --wrap 0 direct-cloud-limited-conservative.yaml)" \
        -p flags="--out_url ${out_url} ${flags} ${extra_flags}" \
        -p gpu-train="true"
}

submit direct-qc-rnn-v1-shared-weights-conservative
