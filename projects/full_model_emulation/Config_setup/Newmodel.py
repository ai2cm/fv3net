...
class SpatioAttenLayer(nn.Module):
    def __init__(self, c, Lk,num_heads): # c : hidden dimension Lk: graph matrix
        super(SpatioAttenLayer, self).__init__()
        self.g = Lk
        self.gc = GATConv(c, c, num_heads)
        self.gc2 = GATConv(c * num_heads, c, 1)
    ...
    def forward(self, x):
        ...
        output = output.view(-1, output.size(1),output.size(2),output.size(3) * output.size(4)) # (in_feat, num_heads, out_dim) -> (in_feat, num_heads * out_dim)
        output = F.elu(output)
        output = self.gc2(self.g, output)
            # Squeeze the head dim as it's = 1 
        output = output.reshape(output.size(0),output.size(1),output.size(2),output.size(4)) # (in_feat, 1, out_dim) -> (in_feat, out_dim)
        output = output.transpose(1, 3)
        output = output.transpose(0, 3)
        return output
...
class FullyConvLayer(nn.Module):
    def __init__(self, c,cout):
        ...
        self.conv = nn.Conv2d(c, cout, 1)

    def forward(self, x):
        return self.conv(x)

class OutputLayer(nn.Module):
    def __init__(self, c,cout,T, n):
        super(OutputLayer, self).__init__()
        self.tconv1 = nn.Conv2d(c, c, (T, 1), 1, dilation = 1, padding = (0,0))
        self.ln = nn.LayerNorm([c])
        self.tconv2 = nn.Conv2d(c, c, (1, 1), 1, dilation = 1, padding = (0,0))
        self.fc = FullyConvLayer(c,cout)
...
class STGCN_WAVE(nn.Module):
    def __init__(self, c, T, out_feat,n, Lk, p, device, num_heads=None, control_str = 'TNTSTNTST'):
        ...
        for i in range(self.num_layers):
            ...
            if i_layer == 'A': # Attention Spatio Layer
                self.layers.append(SpatioAttenLayer(c[cnt], Lk, num_heads))

            if i_layer == 'N': # Norm Layer
                self.layers.append(nn.LayerNorm([c[cnt]]))
        self.output = OutputLayer(c[cnt], out_feat, T + 1 - 2**(diapower), n)
