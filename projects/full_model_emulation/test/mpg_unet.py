...
class MPNNGNN(nn.Module):
    ...
    def __init__(
        ...
        node_hidden_feats,
        edge_hidden_feats,
        num_step_message_passing,aggregator
    ):
...
class UNet(nn.Module):
    ...
    def __init__(
        self, config, down_factory, up_factory, depth: int, node_in_channels: int,edge_in_channels: int
    ):
        """
        Args:
        ...
        node_lower_channels = 2 * node_in_channels

        self._down = down_factory()

        self.conv1 = CubedSphereGraphOperation(
                MPNNGNN(node_in_channels, edge_in_channels, node_lower_channels, config.edge_hidden_feats, config.activation, config.aggregator)
            )
        self.conv2 = CubedSphereGraphOperation(
                MPNNGNN(2*node_lower_channels, edge_in_channels, node_lower_channels, config.edge_hidden_feats, config.activation, config.aggregator)
            )

        if depth == 1:
            self._lower = CubedSphereGraphOperation(
                MPNNGNN(node_lower_channels, edge_in_channels, node_lower_channels * 2, config.edge_hidden_feats, config.activation,config.aggregator)
            )
        elif depth <= 0:
...
class GraphUNet(nn.Module):
    def __init__(self, config, node_in_channels: int, edge_in_channels:int, out_dim: int):
        """
        Args:
            in_channels: number of input channels
        """
        ...
        self._first_layer = CubedSphereGraphOperation(
                MPNNGNN(node_in_channels, edge_in_channels, config.min_filters, config.edge_hidden_feats,config.activation,config.aggregator)
            )

        self._last_layer = CubedSphereGraphOperation(
                MPNNGNN(config.min_filters * 2, edge_in_channels, config.min_filters , config.edge_hidden_feats,config.activation,config.aggregator)
            )
