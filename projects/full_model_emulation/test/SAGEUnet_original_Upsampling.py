...
class UnetGraphSAGE(nn.Module):
    def __init__(
        ...
        self.upsample1 = nn.ConvTranspose2d(
            int(h_feats), int(h_feats / 2), 2, stride=2, padding=0
        )
        self.upsample2 = nn.ConvTranspose2d(
            int(h_feats / 2), int(h_feats / 4), 2, stride=2, padding=0
        )
        self.upsample3 = nn.ConvTranspose2d(
            int(h_feats / 4), int(h_feats / 8), 2, stride=2, padding=0
        )
        self.upsample4 = nn.ConvTranspose2d(
            int(h_feats / 8), int(h_feats / 16), 2, stride=2, padding=0
        )
    ...
    def forward(self, in_feat, exteraVar1):
        ...
        h6 = F.relu(self.conv66(self.g5, h6)).view(
            6,
        ...
        h6 = F.relu(self.conv77(self.g4, h6)).view(
            6,
            int(self.input_res / (self.pooling_size) ** 3),
            int(self.input_res / (self.pooling_size) ** 3),
            -1,
        ...
        h6 = F.relu(self.conv88(self.g3, h6)).view(
            6,
            int(self.input_res / (self.pooling_size) ** 2),
            int(self.input_res / (self.pooling_size) ** 2),
            -1,
        ...
        h6 = F.relu(self.conv99(self.g2, h6)).view(
            6,
            int(self.input_res / (self.pooling_size)),
            int(self.input_res / (self.pooling_size)),
            -1,
