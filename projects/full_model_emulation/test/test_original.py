...
for ss in np.arange(0, int(TotalSamples / Chuncksize)):
    ...
    Z500train = state_training_data[variableList[0]].isel(
        time=slice((ss * Chuncksize), (ss + 1) * Chuncksize)
    )
    T2mtrain1 = state_training_data[variableList[1]].isel(
        time=slice((ss * Chuncksize), (ss + 1) * Chuncksize)
    )
    T2mtrain2 = state_training_data[variableList[2]].isel(
        time=slice((ss * Chuncksize), (ss + 1) * Chuncksize)
    )
