...
savemodelpath='weight_layer_'+control_str+'_lead'+str(lead)+'_window'+str(window)+'_epochs_'+str(epochs)+'.pt'




data_url = "gs://vcm-ml-scratch/ebrahimn/2022-07-02/experiment-1-y/fv3gfs_run/" 
state_training_data= xr.open_zarr(fsspec.get_mapper(os.path.join(data_url, 'atmos_dt_atmos.zarr')))
# state_training_data2 = xr.open_zarr(fsspec.get_mapper(os.path.join(data_url, 'sfc_dt_atmos.zarr')))
lat_lon_data = xr.open_zarr(fsspec.get_mapper(os.path.join(data_url, 'state_after_timestep.zarr')))
...
for ss in np.arange(0,int(TotalSamples/Chuncksize)):
    ...
    loss = nn.MSELoss()
    g = g.to(device)
    model = STGCN_WAVE(channels, int(window/lead), out_feat, num_nodes, g, drop_prob, num_layers, device, control_str).to(device)
