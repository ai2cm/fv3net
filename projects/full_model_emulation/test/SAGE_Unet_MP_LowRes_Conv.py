...
class UnetGraphSAGE(nn.Module):
    def __init__(
        ...
        self.relu=nn.ReLU()
        self.conv = nn.Conv2d(in_feats, 16, (4, 4), stride=(4, 4), padding=0)
        self.convTrans =nn.ConvTranspose2d(60, 32, (4, 4), stride=(4, 4), padding=0)
    ...
    def forward(self, in_feat, edge1,edge2,edge3, edge4, edge5):
        ...
        h1 = torch.permute(h1, (0, 3, 1, 2))
        h1=self.relu(self.conv(h1))
        h1 = torch.permute(h1, (1, 0, 2, 3)).view(
            -1,
            int(
                6
                * self.input_res
        ...
        h6 = torch.permute(h6, (0, 3, 1, 2))
        h6=self.relu(self.convTrans(h6))
        h6 = torch.permute(h6, (1, 0, 2, 3)).reshape(
            -1,
            int(
                6
