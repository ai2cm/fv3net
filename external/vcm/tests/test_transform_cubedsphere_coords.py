...
def test_unit_grid():
    ...
    lont_da = xr.DataArray(
        [[[1.0]]], dims=["tile", "grid_yt", "grid_xt"], coords=centered_coords
    )
    latt_da = xr.DataArray(
        [[[0.0]]], dims=["tile", "grid_yt", "grid_xt"], coords=centered_coords
    )
...
def test_lon_diff(test_unit_grid):
    ...
    lon_diff = _lon_diff(
        test_unit_grid.grid_lon, test_unit_grid.grid_lon.shift(grid_x=-1)
    )[:, :-1, :-1]
    assert lon_diff == 4.0
