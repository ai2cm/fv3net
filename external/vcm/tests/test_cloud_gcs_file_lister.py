...
def test_gcs_lister_all_items():
    ...
    blob_names = ["subdir/blob1.nc", "diff_subdir/new_blob.xyz", "middle_blob.tar"]
...
def test_gcs_lister_use_prefix():
    ...
    subdir_blob_names = ["subdir/blob1.nc", "subdir/blob2.nc"]
    prefix = "subdir/"
...
def test_gcs_lister_file_ext():
    ...
    file_ext_blob_names = ["subdir2/diff_blob.tar", "middle_blob.tar"]
    file_ext = ".tar"
...
def test_gcs_lister_file_ext_and_prefix():
    ...
    blob_names = [
        "subdir/blob1.nc",
        "subdir/blob2.nc",
        "subdir2/diff_blob.tar" "diff_subdir/new_blob.xyz",
        "middle_blob.tar",
        "greater_blob.nc",
    ]

    file_ext_blob_names = ["subdir/blob1.nc", "subdir/blob2.nc"]
    file_ext = ".nc"
    prefix = "subdir/"
    ...
    result = gcs.list_bucket_files(
        fake_client, bucket_name, file_extension=file_ext, prefix=prefix
    )
    assert list(result) == expected
