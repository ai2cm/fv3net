...
def cos_day(self):
    return np.cos(2 * np.pi * self["time"].dt.dayofyear / 366)


@DerivedMapping.register("sin_day")
def sin_day(self):
    return np.sin(2 * np.pi * self["time"].dt.dayofyear / 366)


@DerivedMapping.register("cos_month")
def cos_month(self):
    return np.cos(2 * np.pi * self["time"].dt.month / 12)


@DerivedMapping.register("sin_month")
def sin_month(self):
    return np.sin(2 * np.pi * self["time"].dt.month / 12)
