...
from fv3fit._shared.config import TRAINING_FUNCTIONS
...
def train_identity_model(hyperparameters=None):
    ...
    low, high = 0.0, 1.0
    np.random.seed(0)
    input_variable, output_variables, train_dataset = get_data(size=(time,grid,nz), low=low - 1.0, high=high + 1.0)
    np.random.seed(1)
...
def test_train_default_model_on_identity(regtest):
    ...
    """
    assert_can_learn_identity(max_rmse=0.2, regtest=regtest,
    )
