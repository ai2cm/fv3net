...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(ClipConfig({"var": {"z": SliceConfig(2, 6)}}), np.array([2, 3, 4, 5]), id="clip_1d"),
        pytest.param(ClipConfig({"var": {"z": SliceConfig(8, None)}}), np.array([8, 9]), id="clip_1d_no_stop"),
        pytest.param(ClipConfig({"var": {"z": SliceConfig(None, 2)}}), np.array([0, 1]), id="clip_1d_no_start"),
        pytest.param(ClipConfig({"var_": {"z": SliceConfig(8, None)}}), KeyError, id="clip_var_not_in_config"),
        pytest.param(ClipConfig({"var": {"z": SliceConfig(8, None, 2)}}), NotImplementedError, id="step_slice"),
        pytest.param(ClipConfig({"var": {"z": SliceConfig(8, None), "y": SliceConfig(None, 2)}}), ValueError, id="too_many_clip_dims"),
    ]
)
def test_ClipConfig_clip_layer_along_feature_dim_1d(clip_config, expected):
    ...
    if isinstance(expected, np.ndarray):
        ...
        with pytest.raises(expected):
            clipped = clip_config.clip_1d_layer(layer, "var")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(0, 0, np.array([1., 2.]), id="no_padding_1d"),
        pytest.param(3, 0, np.array([0, 0, 0, 1., 2.]), id="pad_start_1d"),
        pytest.param(0, 3, np.array([1., 2., 0, 0, 0]), id="pad_end_1d"),
        pytest.param(3, 2, np.array([0, 0, 0, 1., 2., 0., 0.]), id="pad_both_1d"),
    ]
)
def test_zero_pad_output_feature_dim_1d(n_pad_start, n_pad_end, expected):
    clipped_layer = tf.constant(np.array([[1., 2.],]))
    filled = zero_pad_output_feature_dim(clipped_layer, n_pad_start, n_pad_end)
    assert np.array_equal(filled.numpy()[0], expected)
...
def test_zero_pad_output_feature_dim_higher_dims(clipped_output_shape, n_pad_start, n_pad_end):
