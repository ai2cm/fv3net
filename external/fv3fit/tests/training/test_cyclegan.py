...
def test_cyclegan_bias_overfit(tmpdir, regtest):
    """
    Test that a "cyclegan" with only a bias layer can overfit to one sample
    """
    fv3fit.set_random_seed(0)
    # run the test in a temporary directory to delete artifacts when done
    os.chdir(tmpdir)
    ...
    hyperparameters = CycleGANHyperparameters(
        ...
        network=CycleGANNetworkConfig(
            generator=fv3fit.pytorch.GeneratorConfig(
                ...
                max_filters=128,
                kernel_size=3,
                disable_convolutions=True,
                use_geographic_bias=True,
            ),
            generator_optimizer=fv3fit.pytorch.OptimizerConfig(
                name="Adam", kwargs={"lr": 0.001}
            ),
        ...
        training=CycleGANTrainingConfig(
            n_epoch=100, samples_per_batch=1, validation_batch_size=2
        ),
    )
    ...
    output_b = predictor.predict(real_a)
    reconstructed_a = predictor.predict(output_b, reverse=True)  # noqa: F841
    for (real, output), label in (
        ((real_a, reconstructed_a), "reconstructed_a"),
        ((real_b, reconstructed_b), "reconstructed_b"),
        ((real_a, output_a), "output_a"),
        ((real_b, output_b), "output_b"),
    ):
        bias = output.isel(time=0) - real.isel(time=0)
        mean_bias: xr.Dataset = bias.mean()
        rmse: xr.Dataset = (bias ** 2).mean() ** 0.5
        for varname in state_variables:
            assert np.abs(mean_bias[varname]) < 0.1, label
            assert rmse[varname] < 0.1, label
