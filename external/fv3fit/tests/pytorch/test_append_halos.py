...
def test_regression(nx: int, ny: int, nz: int, n_tile: int, n_halo: int, regtest):
    """
    Tests only that the result doesn't change.
    """
    np.random.seed(0)
    ds = get_dataset(nx=nx, ny=ny, nz=nz, n_tile=n_tile)
    for name, da in ds.data_vars.items():
        ...
        assert da.sizes.get("z", nz) == nz, name
    append = AppendHalos(n_halo=n_halo)
    result = append(torch.as_tensor(ds.data.values))
    regtest.write(joblib.hash(result.numpy()))
