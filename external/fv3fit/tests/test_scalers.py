...
@pytest.mark.parametrize("shape", [(10, 1), (10, 5), (3, 4, 5)])
...
@pytest.mark.parametrize("shape", [(10, 1), (10, 5), (3, 4, 5)])
def test_standard_scaler_normalize(shape):
    np.random.seed(SEED)
    scaler = StandardScaler()
    X = np.random.uniform(0, 10, size=shape)
    scaler.fit(X)
    ...
    np.testing.assert_almost_equal(
        np.mean(result, axis=tuple(range(len(shape) - 1))), 0
    )
    np.testing.assert_almost_equal(np.std(result, axis=tuple(range(len(shape) - 1))), 1)


@pytest.mark.parametrize("shape", [(10, 1), (10, 5), (3, 4, 5)])
def test_normalize_then_denormalize_on_reloaded_scaler(shape):
    np.random.seed(SEED)
    scaler = StandardScaler()
    X = np.random.uniform(0, 10, size=shape)
    scaler.fit(X)
