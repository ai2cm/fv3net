...
EXPERIMENT=2020-10-12-nn-random-seed-test
mkdir -p configs
...
for i in {0..0}
do
  sed '/random_seed/d' configs/train_config.yml > configs/test_config_seed_$i.yml
  echo "random_seed: $i" >> configs/test_config_seed_$i.yml
  argo submit \
    ...
    -p training-config="$(< configs/test_config_seed_$i.yml)" \
    -p train-routine="keras" \
    -p train-times="$(< train_short.json)" \
    -p test-times="$(< test_short.json)" \
    -p reference-restarts=gs://vcm-ml-experiments/2020-06-02-fine-res/coarsen_restarts \
    -p prognostic-run-config="$(< configs/prognostic_config_test.yml)" \
